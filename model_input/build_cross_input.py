...
def get_train_data(name_list,flag):
    ...
    this_root_path = os.path.join(f'/data/zzp/bei_yi_70/1112_single/{flag}_2')
...
def get_train_data_identity(name_list):
    ...
    this_root_path = os.path.join(f'/data/zzp/bei_yi_70/0502_single/')
    path_list = os.listdir(this_root_path)
    arr_list = []
    all_label = []
    all_softlabel=[]
...
class CNN_INPUT_cross():
    def __init__(self,config):
        ...
        self.pos_data_path = '/data/zzp/bei_yi_70/1107/pos_2_all.pkl'
        self.neg_data_path = '/data/zzp/bei_yi_70/1107/neg_2_all.pkl'
...
class CNN_INPUT_cross_fine_tune():
    ...
    def load_train_data(self):
        ...
        FeatureAll=np.append(pos_train_data.reshape(pos_train_data.shape[0],-1), neg_train_data.reshape(neg_train_data.shape[0],-1) , axis=0)
        print(" FeatureAll", FeatureAll.shape)    
        pos_y = np.ones(pos_train_data.shape[0])
        neg_y = np.zeros(neg_train_data.shape[0])
        allLabel = np.concatenate((pos_y, neg_y), axis=0)    
        ts=TSNE(n_components=3,init='pca',random_state=0)
        visualB=ts.fit_transform(FeatureAll)
        figB=plot_embedding(visualB,allLabel,'')
        #plt.savefig("visualization.png")    
    ...
    def load_test_data(self, patient_id=0):
        ...
        all_scale_way_list = ['add','sub','add_sub','mutiply','random'] * 5
        scale_way_list = all_scale_way_list[:self.pos_train_scale]
        print("scale_way_list",scale_way_list)
...
class CNN_INPUT_cross_identity():
    ...
    def load_train_data(self):
        ...
        train_softlabel_ids = TensorDataset( torch.from_numpy(train_data).cuda(), torch.from_numpy(train_softlabel).cuda())
        # split train/eval data
        ...
        self.train_loader=torch.utils.data.DataLoader(train_data,shuffle=True,batch_size = self.config.batch_size,drop_last = True)
        self.eval_loader=torch.utils.data.DataLoader(eval_data,shuffle=True,batch_size = self.config.batch_size,drop_last = True)
        self.softlabel_train_loader=torch.utils.data.DataLoader(train_softlabel_data,shuffle=True,batch_size = self.config.batch_size,drop_last = True)
        self.softlabel_eval_loader=torch.utils.data.DataLoader(eval_softlabel_data,shuffle=True,batch_size = self.config.batch_size,drop_last = True)
       
        

    
class CNN_INPUT_cross_fine_tune_raw():
    def __init__(self, config):
        self.config = config 
    ...
    def load_train_data(self):
        ...
        if self.pos_train_scale > 0:
            print(f'use scale is {self.pos_train_scale}, scale_way_list is {scale_way_list} ')
            pos_train_data = build_scale_together(pos_train_data, scale_way_list)
        self.pos_train_num = pos_train_data.shape[0]
        self.neg_train_num = neg_train_data.shape[0]
        self.train_loader = self.build_data_iterator(pos_train_data, neg_train_data, 'train')
        self.eval_loader = self.build_data_iterator(pos_eval_data, neg_eval_data, 'eval')
...
class CNN_INPUT_cross_gaussianMix():
    def __init__(self, config):
        self.config = config 
    ...
    def load_train_data(self):
        ...
        pos_train_data= multiGaussianGenerate(pos_train_data,pos_train_data.shape[0])#multiGaussianGenerate
        neg_train_data= multiGaussianGenerate(neg_train_data,neg_train_data.shape[0])
        # split train/eval data
        pos_train_data, pos_eval_data = train_test_split(pos_train_data, test_size=0.2, random_state=42)
        neg_train_data, neg_eval_data = train_test_split(neg_train_data, test_size=0.2, random_state=42)
