...
class SNNIDEConvNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.classifier2 = nn.Linear(self.h_hidden * self.w_hidden * self.c_hidden, self.identiClasses, bias=True)
        '''
        if self.DANNType==1:    
          self.classifier2 = nn.Linear(self.h_hidden * self.w_hidden * self.c_hidden, self.identiClasses, bias=True)
        elif self.DANNType==2:
          self.classifier2 = nn.Sequential(  #domain adversary domain classifier
           nn.Linear(self.h_hidden * self.w_hidden * self.c_hidden, 100),
           nn.BatchNorm1d(100),
           nn.ReLU(True),
