...
class SNNIDEConvMultiLayerNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.classifier = nn.Linear(115200, self.num_classes, bias=True)
        #6400, self.num_classes, bias=True)#reshape结构
       # self.classifier = nn.Linear(self.c_s4 * self.h_hidden * self.w_hidden, self.num_classes, bias=True)
        ...
        self.classifier2 = nn.Sequential(  #domain adversary domain classifier
          nn.Linear(115200,100),#reshape结构#(self.c_s4 * self.h_hidden * self.w_hidden, 100),
          nn.BatchNorm1d(100),
          nn.ReLU(True),
    ...
    def _forward(self, x, **kwargs):
        ...
        x1 = torch.zeros([B, self.c_s2, (H//self.stride_x+1)//2, (W//self.stride_x+1)//2]).to(x.device)
        self.network_s2._reset(x1)
        x1 = torch.zeros([B, self.c_s3, (H//self.stride_x+1)//2, (W//self.stride_x+1)//2]).to(x.device)
        self.network_s3._reset(x1)
        x1 = torch.zeros([B, self.c_s4, (H//self.stride_x+1)//2, (W//self.stride_x+1)//2]).to(x.device)
