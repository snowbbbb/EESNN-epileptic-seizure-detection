...
from modules.snn_modules import SNNIFFuncMultiLayer, SNNLIFFuncMultiLayer, SNNConv, SNNConvTranspose,SNNZero
...
class SNNIDEConvMultiLayerNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.network_s2 = SNNConv(self.c_s1, self.c_s2, self.kernel_size_s, bias=True, BN=True, stride=(2,1), pooling=False, dropout=self.dropout)
        ...
        self.network_s5 = SNNZero(input_size=[1, 688128], output_size=[1, self.c_hidden, self.h_hidden, self.w_hidden])#SNNConvTranspose(self.c_s4, self.c_hidden, bias=False, dropout=self.dropout, kernel_size=3, stride=(2,1), padding=1, output_padding=(1,0))
        ...
        self.classifier = nn.Linear(688128 , self.num_classes, bias=True)#6400, self.num_classes, bias=True)#reshape结构
        
        
        self.classifier2 = nn.Sequential(  #domain adversary domain classifier
          nn.Linear(688128 , 100),#6400,100),#reshape结构
          nn.BatchNorm1d(100),
          nn.ReLU(True),
          nn.Linear(100,self.identiClasses), )
        '''
        if self.DANNType==1:    
          self.classifier2 = nn.Linear(self.c_s4 * self.h_hidden * self.w_hidden, self.identiClasses, bias=True)#6400, self.identiClasses, bias=True)#reshape结构
        elif self.DANNType==2:
          self.classifier2 = nn.Sequential(  #domain adversary domain classifier
          nn.Linear(self.c_s4 * self.h_hidden * self.w_hidden, 100),#6400,100),#reshape结构
          nn.BatchNorm1d(100),
          nn.ReLU(True),
          nn.Linear(100,self.identiClasses), )
        '''
    ...
    def parse_cfg(self, cfg):
        ...
        self.identiClasses = cfg['MODEL']['identiClasses']
       # self.DANNType=cfg['MODEL']['DANNType'] 
        self.input_type=cfg['MODEL']['input_type'] #if 'input_type' in cfg['MODEL'].keys() else 'constant'
    ...
    def _forward(self, x, **kwargs):
        ...
        x1 = torch.zeros([B, self.c_s2, (H//self.stride_x+1)//2, W//self.stride_x]).to(x.device)
        self.network_s2._reset(x1)
        x1 = torch.zeros([B, self.c_s3, (H//self.stride_x+1)//2, W//self.stride_x]).to(x.device)
        self.network_s3._reset(x1)
        x1 = torch.zeros([B, self.c_s4, (H//self.stride_x+1)//2, W//self.stride_x]).to(x.device)
    ...
    def forward(self, x,forward_type='normal', feature_grad=True, **kwargs):
        #print("x.size",x.size())
