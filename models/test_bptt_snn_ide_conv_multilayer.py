...
import copy
sys.path.append("../")
from modules.test_bptt_snnide_multilayer_module import SNNIDEMultiLayerModule
from modules.test_bptt_snn_modules import SNNIFFuncMultiLayer, SNNLIFFuncMultiLayer, SNNConv, SNNConvTranspose
...
class SNNIDEConvMultiLayerNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.classifier = nn.Linear(344064, self.num_classes, bias=True)
    ...
    def parse_cfg(self, cfg):
        ...
        self.dropout = cfg['MODEL']['dropout'] if 'dropout' in cfg['MODEL'].keys() else 0.2
    ...
    def _forward(self, x, **kwargs):
        ...
        z = self.snn_ide_conv(z, x, time_step=time_step, threshold=f_thres, input_type=input_type, solver_type=self.solver, leaky=leaky)
    ...
    def get_all_rate(self, x, **kwargs):
        ...
        z = torch.zeros([B, self.c_hidden, H//2, W//2]).to(x.device)
        r_list = self.snn_ide_conv(z, x, time_step=time_step, threshold=f_thres, input_type=input_type, solver_type=self.solver, leaky=leaky, get_all_rate=True)
