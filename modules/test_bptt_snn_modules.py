...
import os
sys.path.append('../')
from modules.optimizations import weight_norm, VariationalHidDropout2d, weight_spectral_norm


class StepFunctionGrad(Function):
    @staticmethod
    ...
    def backward(ctx, grad):
        ...
        x = ctx.x
        vth = ctx.vth
        mask = (torch.abs(x - vth) < a1 / 2).float() * a1
...
class StepFunction(nn.Module):
    ...
    def forward(self, input, vth):
        return StepFunctionGrad.apply(input, vth)
...
class SNNFunc(nn.Module):
    ...
    def snn_forward(self, x, time_step, output_type='firing_rate', input_type='constant', get_all_rate=False):
        pass
    ...
    def copy_bn_stat(self, target):
        self.network_x.copy_bn_stat(target.network_x)
        self.network_s.copy_bn_stat(target.network_s)
...
class SNNIFFunc(SNNFunc):
    ...
    def snn_forward(self, x, time_step, output_type='firing_rate', input_type='constant', get_all_rate=False):
        ...
        u = x1
        #s = (u >= self.vth).float()
        s = self.StepFunction(u, self.vth)
        u = u - self.vth * s

        a = s
        if output_type != 'firing_rate':
            sizes = s.size()
            shape = [time_step]
...
class SNNFuncMultiLayer(nn.Module):
    ...
    def snn_forward(self, x, time_step, f_type='first', input_type='constant', get_all_rate=False):
        pass
    ...
    def equivalent_func(self, a, x, f_type='first'):
        if f_type == 'first':
    ...
    def copy_bn_stat(self, target):
        self.network_x.copy_bn_stat(target.network_x)
        for i in range(len(self.network_s_list)):
            self.network_s_list[i].copy_bn_stat(target.network_s_list[i])
...
class SNNIFFuncMultiLayer(SNNFuncMultiLayer):
    ...
    def snn_forward(self, x, time_step, f_type='first', input_type='constant', get_all_rate=False):
        ...
        s1 = self.StepFunction(u1, self.vth)
        u1 = u1 - self.vth * s1
        u_list.append(u1)
        s_list.append(s1)
        for i in range(len(self.network_s_list) - 1):
            ...
            si = self.StepFunction(ui, self.vth)
            ui = ui - self.vth * si
            u_list.append(ui)
            s_list.append(si)
        ...
        for t in range(time_step - 1):
            ...
            s_list[0] = self.StepFunction(u_list[0], self.vth)
            ...
            for i in range(len(self.network_s_list) - 1):
                ...
                s_list[i + 1] = self.StepFunction(u_list[i + 1], self.vth)
...
class SNNLIFFuncMultiLayer(SNNFuncMultiLayer):
    ...
    def snn_forward(self, x, time_step, f_type='first', input_type='constant', get_all_rate=False):
        ...
        if f_type == 'first':
            return af / ((1. - self.leaky ** time_step) / (1. - self.leaky))
        elif f_type == 'last':
            return al / ((1. - self.leaky ** time_step) / (1. - self.leaky))
        else:
...
class SNNFC(nn.Module):
    ...
    def _wnorm(self):
        self.fc, self.fc_fn = weight_spectral_norm(self.fc, names=['weight'], dim=0)
...
class SNNConv(nn.Module):
    ...
    def __init__(self, d_in, d_out, kernel_size, bias=False, BN=False, stride=1, padding=None, pooling=False, dropout=0.0):
        super(SNNConv, self).__init__()
        if padding == None:
            padding = (kernel_size - 1) // 2
        self.conv = nn.Conv2d(d_in, d_out, kernel_size, stride, padding, bias=True)
        self.BN = BN
    ...
    def _wnorm(self):
        self.conv, self.conv_fn = weight_spectral_norm(self.conv, names=['weight'], dim=0)
...
class SNNConvTranspose(nn.Module):
    ...
    def _wnorm(self):
        self.convT, self.convT_fn = weight_spectral_norm(self.convT, names=['weight'], dim=0)
