...
from model_input.build_cross_input import CNN_INPUT_cross, CNN_INPUT_cross_fine_tune,CNN_INPUT_cross_gaussian,CNN_INPUT_cross_identity
...
from models.snn_ide_conv_multilayer_DANN import SNNIDEConvMultiLayerNet
...
def train_once(model,optimizer,train_iterator,params,epoch_i):
    ...
    for i,(batch_x,batch_y) in enumerate(train_iterator):
        ...
        model_input=model_input.permute(0,4,3,1,2)
...
def train_once_DANN(model,optimizer,optimizer2,train_iterator,train_identity_dataloader,len_train_iter,params,epoch_i):
    ...
    for i in range(len_train_iter):
        ...
        model_input_iden=model_input_iden.view(BATCH_SIZE, 1,time_window_size,FREQUENCY,-1)# [batchsize, 1, 2,50, 18]
        model_input_iden=model_input_iden.permute(0,4,3,1,2)
...
def run_test_once(best_model):
    ...
    for i in range(test_num):
        ...
        test_result_dic = {}
        accuracy, precision, recall, specificity, train_auc, train_loss, fine_tune_model = train_once(model, optimizer,
                                                                                                      fine_tune_loader,
                                                                                                      params, i)
        fine_tune_epochs = 10
        ...
        for k in range(fine_tune_epochs):
            accuracy, precision, recall, specificity, train_auc, train_loss, fine_tune_model = train_once(
                fine_tune_model, optimizer, fine_tune_loader, params, i)
            if specificity >= best_specificity and recall >= best_recall:
                best_recall = recall
                best_specificity=specificity
                torch.save(fine_tune_model,best_fine_model_path)
                #best_fine_model = copy.deepcopy(fine_tune_model)
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(cuda_id):
        ...
        config['MODEL']['identiClasses']=10
        config['OPTIM'] = {}
        config['OPTIM']['solver'] = 'broy'
        config['MODEL']['input_type'] = 'nonconstant'
        #config['MODEL']['DANNType'] = params.DANNType
