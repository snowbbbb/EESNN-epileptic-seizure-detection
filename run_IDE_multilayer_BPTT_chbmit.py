...
from model_input.build_cross_input_mit4s_noFT import CNN_INPUT_cross_gaussian, \
    CNN_INPUT_cross_identity  # from model_input.build_cross_input_mit5s_noFT import CNN_INPUT_noFT, CNN_INPUT_cross, CNN_INPUT_cross_fine_tune,CNN_INPUT_cross_gaussian
...
from models.test_bptt_snn_ide_conv_multilayer import SNNIDEConvMultiLayerNet
...
def train_once_DANN(model, optimizer, optimizer2, train_iterator, train_identity_dataloader, len_train_iter, params,
                    epoch_i):
    # print(f'-- epoch is {epoch_i}, cnn model train begin -- ')
    model.train()
    train_loss = 0.0
...
def evaluate(model, eval_iterator, params, epoch_i, run_mode='eval'):
    ...
    for i, (batch_x, batch_y) in enumerate(eval_iterator):
        ...
        predict_y, _ = torch.max(out, dim=1)
        loss = loss_function(predict_y, batch_y)
        batch_r_list = model.get_all_rate(model_input)
        total_dims = 0
        batch_r = 0.
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(cuda_id):
        ...
        config['MODEL']['dropout'] = 0#0.2
        config['MODEL']['leaky'] = 0.95
        config['MODEL']['f_thres'] =0# 30
        ...
        best_pretrain_model_path = '/data/zzp/result/model_save/mit/resnet_scale0_epoch0of1_07311631.pkl'#resnet_scale0_epoch46of50_02231005.pkl'
