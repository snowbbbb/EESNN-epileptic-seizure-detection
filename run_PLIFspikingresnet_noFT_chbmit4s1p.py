...
from model_input.build_cross_input_mit4s_noFT import CNN_INPUT_cross, CNN_INPUT_cross_fine_tune,CNN_INPUT_cross_gaussian
...
def train_once(model,optimizer,train_iterator,params,epoch_i):
    ...
    for i,(batch_x,batch_y) in enumerate(train_iterator):
        ...
        model_input=model_input.permute(2,0,1,3,4)#(2,0,4,3,1)
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(cuda_id):
        model = multi_step_spiking_resnet18(T=10, multi_step_neuron=neuron.ParametricLIFNode, surrogate_function=surrogate.ATan(), detach_reset=False)
        model = model.cuda()
