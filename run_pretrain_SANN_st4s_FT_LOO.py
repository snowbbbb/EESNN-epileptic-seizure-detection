...
from model_input.build_cross_input_mit4s_FT_LOO import CNN_INPUT_cross, CNN_INPUT_cross_identity, CNN_INPUT_cross_fine_tune,CNN_INPUT_cross_gaussian
...
def run_train(model, optimizer,params,patientId,use_eval=1):
    ...
    for i in range(EPOCHS):
        ...
        if use_eval:
            ...
            if all_score > best_score:
                model_path = os.path.join(model_root_path,f'pretrain_{start_time_tag}_{patientId}.pkl')
    ...
    best_model=torch.load(f'/data/zzp/result/model_save/mit/pretrain_{start_time_tag}_{patientId}.pkl')     
...
def run_test_once(best_model):
    ...
    for i in range(test_num):
        ...
        test_result_dic = {}
        accuracy, precision, recall, specificity, train_auc, train_loss, fine_tune_model = train_once(best_model, optimizer,
                                                                                                      fine_tune_loader,
                                                                                                      params, i)
        fine_tune_epochs = 10
        best_fine_model_path = os.path.join(model_root_path,f'finetune_bestmodel_{start_time_tag}.pkl')
        torch.save(fine_tune_model,best_fine_model_path)
        #best_fine_model = copy.deepcopy(fine_tune_model)
        #way1
        best_train_score = 1.5*recall + specificity + train_auc
        for k in range(fine_tune_epochs):
            accuracy, precision, recall, specificity, train_auc, train_loss, fine_tune_model = train_once(
                fine_tune_model, optimizer, fine_tune_loader, params, i)
            tmp_score = 1.5*recall  + specificity + train_auc
            if tmp_score > best_train_score:
                best_train_score = tmp_score
                torch.save(fine_tune_model,best_fine_model_path)
                #best_fine_model = copy.deepcopy(fine_tune_model)
        best_fine_model=torch.load(f'/data/zzp/result/model_save/mit/finetune_bestmodel_{start_time_tag}.pkl')
        test_accuracy, test_precision, test_sensitivity, test_specificity, test_auc, _ = evaluate(best_fine_model,
                                                                                                  test_loader, params,
                                                                                                  run_mode='test',
                                                                                                  epoch_i=i)
...
if __name__ == '__main__':
    ...
    fid = open(f'SNN_ExperimentResult_LOO{params.lr}.txt', 'w')
    #score_id_list =['chb12','chb13','chb14','chb15','chb21']
    score_id_list =['chb12','chb13','chb14','chb05']
    cuda_id = params.cuda
        ...
        for pid in range(len(patient_id_list)):
            ...
            with torch.cuda.device(cuda_id):
                ...
                if params.optimizer == 'Adam':
                    ...
                    optimizer = torch.optim.SGD(module_list, lr=params.lr, momentum=0.9, weight_decay=5e-4)
                    optimizer2 = torch.optim.SGD(model.classifier2.parameters(), lr=params.lr, momentum=0.9,
                                                 weight_decay=5e-4)
                best_pretrain_model_path = run_train(model, optimizer=optimizer, params=params,use_eval=1,patientId=patient_id_list[pid])
                #best_pretrain_model_path='/data/zzp/result/model_save/mit/pretrain_03180958_chb01.pkl'
                best_model = torch.load(best_pretrain_model_path)
                print('log_path:',log_path)
