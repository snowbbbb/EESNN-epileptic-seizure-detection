...
from model_input.build_cross_input_beiyi2s_noFT import CNN_INPUT_cross, CNN_INPUT_cross_fine_tune,CNN_INPUT_cross_gaussian
...
from models.model_spiking_resnet import multi_step_spiking_resnet18
...
def train_once(model,optimizer,train_iterator,params,epoch_i):
    ...
    for i,(batch_x,batch_y) in enumerate(train_iterator):
        ...
        model_input=model_input.view(BATCH_SIZE, 1,10 ,10,-1)
        #model_input=model_input.view(BATCH_SIZE, 1,time_window_size,FREQUENCY,-1)# [batchsize, 1, 2,50, 18]
        #model_input=model_input.permute(2,0,4,3,1)#[T, N, C, H, W]
        model_input=model_input.permute(2,0,1,3,4)
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(cuda_id):
        model = multi_step_spiking_resnet18(T=10, multi_step_neuron=neuron.MultiStepLIFNode, tau=10., surrogate_function=surrogate.ATan(), detach_reset=False)
        model = model.cuda()
